...
from protein_lm.data.task7_fixed_budget import compare_caps, summarize_cap
...
def test_compare_caps_accepts_an_escalated_query_subset(tmp_path: Path) -> None:
    ...
    with pytest.raises(SimilarityAuditError, match="frozen A-004 stages"):
        compare_caps(
            baseline_cap=1_000,
            comparison_cap=100_000,
            baseline_canonical_path=common,
    ...
    with pytest.raises(SimilarityAuditError, match="summary values"):
        compare_caps(
            baseline_cap=10_000,
            comparison_cap=100_000,
            baseline_canonical_path=common,
