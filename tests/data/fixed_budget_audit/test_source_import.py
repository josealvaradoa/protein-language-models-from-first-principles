...
from protein_lm.data.fixed_budget_audit.source import verify_a003_residual_import
...
def test_import_rejects_wrong_escalation_membership(tmp_path: Path) -> None:
    ...
    with pytest.raises(SourceEvidenceError, match="differs from cap changes"):
        verify_a003_residual_import(project_root=root, policy=policy)
...
def _build_fixture(
    ...
    *,
    ...
    escalation_path = (
        workspace / "tracks/random/validation/residual/escalated_queries.fasta"
    )
