"""Readable top-level workflow for the historical diagnostic audit."""
...
from protein_lm.data.fixed_budget_audit.errors import AuditPublicationError
...
def run_diagnostic_similarity_audit(
    *,
    ...
    for output_path in (
        *(report_directory / filename for filename in DIAGNOSTIC_REPORT_FILENAMES),
        report_directory / DIAGNOSTIC_COMPLETION_FILENAME,
    ):
        prove_path_is_public(output_path, project_root)
    if (report_directory / DIAGNOSTIC_COMPLETION_FILENAME).exists():
...
def _audit_strategy(
    *,
    ...
    partitions: dict[str, object] = {"training": {"balance": balances["training"]}}
    ...
    return (
        {
            ...
            "structural_membership": diagnostic_structural_membership_evidence(
                strategy,
                manifest,
                frozen_report,
            ),
            "partitions": partitions,
            "overall": diagnostic_overall_similarity(partitions),
        },
        database_report,
    )
...
def _audit_partition(
    *,
    ...
    return {
        ...
        "passes": {
            pass_name: diagnostic_public_pass_evidence(pass_reports[pass_name])
            for pass_name in ("enforcement", "residual")
        },
    }
