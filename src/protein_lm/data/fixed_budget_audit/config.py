"""Frozen configuration and core vocabulary for the fixed-budget audit."""
...
class SplitStrategy(StrEnum):
    """Frozen split strategies compared by the audit."""
...
class DatasetPartition(StrEnum):
    """Held-out dataset partitions inspected by the audit."""
...
class AuditPass(StrEnum):
    """Similarity-search passes contributing audit evidence."""

    ENFORCEMENT = "enforcement"
    RESIDUAL = "residual"


class TrackOrigin(StrEnum):
    """Whether a track is imported from A-003 or freshly executed for A-004."""

    IMPORTED_A003 = "imported_a003"
    EXECUTED_A004 = "executed_a004"


class PairUnionKind(StrEnum):
    """The two distinct prohibited-pair result semantics."""

    COMMON_ALL_QUERY_10000 = "common_all_query_10000"
    STAGED_UNION_WITH_CHANGED_QUERY_100000 = "staged_union_with_changed_query_100000"


class QueryScope(StrEnum):
    """Wire vocabulary for all-query and changed-query cap evidence."""

    ALL_QUERIES = "all_queries"
    CHANGED_QUERIES_1000_TO_10000 = "changed_queries_1000_to_10000"


class CandidateCap(IntEnum):
    """Role names for the three approved candidate caps."""
...
FIXED_CAPS: tuple[int, ...] = tuple(cap.value for cap in CandidateCap)
...
class A004Policy:
    ...
    all_query_cap: CandidateCap
    staged_escalation_cap: CandidateCap
    import_strategy: SplitStrategy
    import_partition: DatasetPartition
    import_pass: AuditPass
    ...
    def stage_marker_sha256(self, cap: int) -> str:
        ...
        try:
            ...
            raise AuditConfigurationError(
                f"cap is not frozen for A-004: {cap}"
            ) from error
...
def load_a004_policy(path: Path) -> A004Policy:
    ...
    try:
        ...
        raise AuditConfigurationError(
            f"could not load A-004 policy: {error}"
        ) from error

    if hashlib.sha256(content).hexdigest() != APPROVED_A004_CONFIG_SHA256:
        raise AuditConfigurationError(
            "A-004 policy bytes do not match the approved checksum"
        )
    if set(raw) != {field.name for field in fields(A004Policy)}:
        raise AuditConfigurationError(
            "A-004 policy fields differ from the approved schema"
        )
    ...
    try:
        ...
        raise AuditConfigurationError(f"invalid A-004 policy: {error}") from error
...
def resolve_a004_paths(policy: A004Policy, project_root: Path) -> dict[str, Path]:
    ...
    if workspace.is_relative_to(source_workspace) or source_workspace.is_relative_to(
        ...
        raise AuditConfigurationError(
            "A-004 must use a non-overlapping sibling workspace"
        )
    return resolved
...
def _stage_marker_pins(raw: object) -> tuple[tuple[int, str], ...]:
    if not isinstance(raw, dict) or set(raw) != {str(cap) for cap in FIXED_CAPS}:
        raise AuditConfigurationError(
            "A-004 stage-marker caps differ from the fixed caps"
        )
...
EnumValue = TypeVar("EnumValue", SplitStrategy, DatasetPartition, AuditPass)
...
def _typed_value(value: object, name: str, annotation: object) -> object:
    ...
    if enum_type is not None:
        return _string_enum_value(value, name, enum_type)
    if str(annotation) == "CandidateCap":
        if isinstance(value, bool):
            raise TypeError(f"{name} must be an integer")
        if not isinstance(value, int):
            raise TypeError(f"{name} must be int")
        return CandidateCap(value)
...
def _validate_policy(policy: A004Policy) -> None:
    required = {
        ...
        "reporting caps": (
            (policy.all_query_cap.value, policy.staged_escalation_cap.value),
    ...
    if drift:
        raise AuditConfigurationError("A-004 authority drift: " + "; ".join(drift))
    ...
    for name in (
        ...
        if path.is_absolute() or ".." in path.parts or path.as_posix() != value:
            raise AuditConfigurationError(f"{name} must stay inside the repository")
    if policy.source_workspace_relative_path == policy.workspace_relative_path:
        raise AuditConfigurationError("A-004 must use a sibling workspace")
    ...
    if len(policy.source_code_revision) != 40:
        raise AuditConfigurationError(
            "source_code_revision must be a full Git commit hash"
        )
...
def _require_sha256(value: object, name: str) -> None:
    if not isinstance(value, str) or len(value) != 64:
        raise AuditConfigurationError(f"{name} must be a lowercase SHA-256")
    _require_hex(value, name)


def _require_hex(value: str, name: str) -> None:
    if any(character not in "0123456789abcdef" for character in value):
        raise AuditConfigurationError(f"{name} must use lowercase hexadecimal")
