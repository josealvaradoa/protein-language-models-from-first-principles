"""Operating-system and process boundaries for fixed-budget audits."""
...
def require_disk_capacity(workspace: Path, policy: SimilarityAuditPolicy) -> None:
    ...
    if workspace_size > policy.workspace_byte_ceiling:
        raise AuditExecutionError("Task 7 workspace exceeded its fixed 200 GiB ceiling")
...
def exclusive_lock(path: Path) -> Iterator[None]:
    ...
    with path.open("a+") as lock:
        try:
            ...
            raise AuditExecutionError(
                "another Task 7 audit is already running"
            ) from error
        lock.seek(0)
        lock.truncate()
...
def prove_path_is_ignored(path: Path, project_root: Path) -> None:
    ...
    if result.returncode != 0:
        raise AuditExecutionError(f"private path is not ignored by Git: {relative}")
...
def prove_path_is_public(path: Path, project_root: Path) -> None:
    ...
    if result.returncode == 0:
        raise AuditExecutionError(f"public path is unexpectedly ignored: {relative}")
    if result.returncode != 1:
        raise AuditExecutionError(f"could not prove public Git status: {relative}")
